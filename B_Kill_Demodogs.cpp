...
int modDiv(int a, int b, int mod) {
    int b_inv = binpow(b, mod - 2);  // Modular inverse using Fermat's theorem
...
int32_t main() {
    ...
    while (t--) {
        ...
        ans += modDiv(modMul(modMul(n, (n + 1), MOD), (2 * n + 1), MOD), 6, MOD);
        ans += modDiv(modMul(modMul(n, (n - 1), MOD), (2 * n - 1), MOD), 6, MOD);
        ans += modDiv(modMul(n, (n - 1), MOD), 2, MOD);
        ans = modMul(ans, 2022, MOD);
