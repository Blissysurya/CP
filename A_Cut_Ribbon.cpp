...
int32_t main(){
    ...
    for(int i=minel;i<=n;i++){
        if((i-a)>=0 && (i-b)>=0 && (i-c)>=0){
            if(dp[i-a]>=1 || dp[i-b]>=1 || dp[i-c]>=1){
            dp[i]=max(dp[i-a]+1,max(dp[i-b]+1,dp[i-c]+1));
            }
        }else if((i-a)>=0 && (i-b)>=0){
            ...
            if( dp[i-b]>=1 || dp[i-c]>=1){
            dp[i]=max(dp[i-b]+1,dp[i-c]+1);
            }
        }else if((i-a)>=0 && (i-c)>=0){
            if(dp[i-a]>=1 || dp[i-c]>=1){
            dp[i]=max(dp[i-a]+1,dp[i-c]+1);
            }
        }
        else if((i-a)>=0){
            if(dp[i-a]>=1 ){
