...
void dijkstra(vector<int>& par,vector<int> &dist,vector<vector<pair<int,int>>>& graph ){
    int n=graph.size();
    par.resize(n,-1);
    ...
    while(!s.empty()){
        ...
        for(auto v: graph[u]){
            if(dist[v.second] > dist[u]+ v.first ){ 
                s.erase({dist[v.second],v.second});
                dist[v.second] = dist[u]+ v.first;
                par[v.second]=u;
                s.insert({dist[v.second],v.second});
            }
        }
