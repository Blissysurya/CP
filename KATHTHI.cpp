        ...
        while(!dq.empty()){
            ...
            for(int i=0;i<4;i++){
                if((x+dx[i])<n && (x+dx[i])>=0 && (y+dy[i])<m && (y+dy[i])>=0){
                    if(ch==vec[x+dx[i]][y+dy[i]]){
                        if(dist[x+dx[i]][y+dy[i]] > dist[x][y]){
                            dist[x+dx[i]][y+dy[i]]=dist[x][y];
                            dq.push_front({vec[x+dx[i]][y+dy[i]],{x+dx[i],y+dy[i]}});
                        }
                       
                    }else{
                        if(dist[x+dx[i]][y+dy[i]] > dist[x][y]+1){
                            dist[x+dx[i]][y+dy[i]]=dist[x][y]+1;
                            dq.push_back({vec[x+dx[i]][y+dy[i]],{x+dx[i],y+dy[i]}});
                        }
